...
Secrets use the same typed update-side shape without an app-specific Keychain bridge. `fx.setCredential`, `fx.getCredential`, and `fx.deleteCredential` address an OS credential by the stable `(service, account)` pair and deliver one `EffectCredentialResult`:
...
.credential_result => |result| switch (result.outcome) {
    .ok => model.useToken(result.secret), // valid only during this update

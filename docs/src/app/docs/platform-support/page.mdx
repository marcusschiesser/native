...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Menus</td>
      <td><Tier tier="full" note="native app menus and native context menus (NSMenu)" fn={5} /></td>
      <td><Tier tier="full" note="native app menus and native context menus (TrackPopupMenu)" fn={5} /></td>
      <td><Tier tier="full" note="native app menus and native context menus (GtkPopoverMenu)" fn={5} /></td>
...
Audio capture is intentionally narrower than playback: only the macOS 15+ system-engine host exposes `system_audio_capture`, `microphone_capture`, and `microphone_device_enumeration`. The feature flags stay false on older macOS and every Chromium, Linux, Windows, iOS, and Android host. See [Audio Capture](/docs/audio-capture) for permissions, source selection, device identity, and the reliable stream contract.

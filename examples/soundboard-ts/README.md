...
- **Press plays.** This TypeScript variant keeps the touch convention — one press plays — while Native markup also supports the desktop `on-press` select + `on-double-press` play pairing. The arrow-key selection register goes with the one-press design; since the original's play gesture also selects, the LOADED row wears the same inverted accent register in both apps, and the two are visually 1:1 after any play.

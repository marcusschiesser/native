...
### The streaming ops: spawn, playback, capture, and channels
...
- `Cmd.captureAccess(key, source, action, { event })` — query (`action: "status"`) or explicitly request (`"request"`) `"system_audio"` or `"microphone"` access. Starting capture never prompts. The event arm is matched by NAME: `key: Uint8Array`, `source`, `status`, and `restartRequired`. System audio reports `"authorized" | "not_authorized" | "unavailable"`; microphone can also report `"not_determined" | "denied" | "restricted"`. Surface `restartRequired` after a system-audio request instead of assuming the running process can capture immediately.
- `Cmd.microphoneDevices(key, { event })` — stream a microphone snapshot as zero or more `"device"` events plus exactly one `"completed" | "failed" | "rejected"` terminal. The event arm carries `key: Uint8Array`, `state`, `id`, `name`, `isDefault`, `index`, and `total`; retain device records in the Model if the view needs them. Pair it with `Sub.microphoneDevicesChanged("devices_changed")`, an invalidation signal that means list again.
- `Cmd.audioCaptureStart(key, options, { event })` — open the app's single reliable PCM capture stream. Options select system audio, `"none" | "default" | Uint8Array` microphone identity, sample rate, channels, current-process exclusion, and the bounded buffer duration. The lifecycle arm carries `key: Uint8Array`, `state`, `reason`, `sampleRate`, `channels`, `availableFrames`, `capacityFrames`, and `framesProduced`; `"readable"` is coalesced, so it means drain until `remainingFrames` reaches zero, not one event per chunk.
- `Cmd.audioCaptureRead(key, maxFrames, { event })` — pull up to 100 ms of whole aligned blocks; keep at most one read outstanding. The read arm carries `key: Uint8Array`, `state`, `reason`, `sequence`, `frameOffset`, `frames`, `systemPcm`, `microphonePcm`, `systemGapFrames`, `microphoneGapFrames`, `remainingFrames`, and `endOfStream`. The signed 16-bit little-endian PCM byte slices are BORROWED for that update only: meter, encode, transcribe, or copy them before returning; never store the borrowed slices directly in the Model.
- `Cmd.audioCaptureStop(key)` stops producers but preserves accepted frames: keep reading after the `"stopped"` lifecycle event until `endOfStream`. `Cmd.audioCaptureDiscard(key)` stops and releases immediately, dropping buffered PCM. The bounded ring never overwrites accepted data; a consumer that does not drain seals the stream as `"failed" / "consumer_too_slow"`, with accepted frames still available to read.
...
- `Sub.microphoneDevicesChanged("devices_changed")` — microphone-list invalidation. The named Msg arm has no payload; issue `Cmd.microphoneDevices` from that update to obtain a fresh snapshot.
...
- **Drag (`on-drag`, the live insertion event):** write a bound constructor such as `<row global-key="{card.id}" on-drag="card_dragged:{card.id}">`. Its Msg arm must be exactly `{ kind: "card_dragged"; sourceId: number; phase: number; x: number; y: number; viewWidth: number; viewHeight: number }`. Markup fills `sourceId`; the runtime fills view-local geometry and `phase` (`0` change, `1` end, `2` cancel). The renderer lifts the source appearance under the pointer at full opacity and leaves its in-flow rendering blank. For precise reordering, keep committed data unchanged during change and have derived array helpers move the same globally keyed source record to the candidate slot. That hidden row is the ONE reserved card-sized space: it starts at the source and moves to the candidate instead of duplicating. Keyed draggable neighbors ease between candidate poses; commit the array order on end and clear the candidate on cancel. A plain Escape during the drag is consumed and dispatches phase 2; the floating item continues from its pointer position into the committed slot (or back on cancel), and reduced-motion appearances snap the reflow.
...
- **`dropMsg(drop: FileDropEvent): Msg | null`** — native file drops after widget routing. `FileDropEvent` is exactly `{ windowId; viewLabel; point; paths }`: `point` is `{ x; y } | null`, and `paths` is `readonly Uint8Array[]` so arbitrary UTF-8 filesystem names remain byte text. Return null to ignore the drop or map it to any ordinary Msg.
